...
def extract_card_id(raw_swipe: str) -> str:
    """
    Adjust this function once you confirm the exact MiniMag output format.

    Current strategy:
    1. Strip whitespace
    2. Look for long digit groups and take the last one
    3. Fall back to the cleaned raw string

    Examples:
    - '%B1234567890123456^DOE/JOHN^...?' -> '1234567890123456'
    - ';12345=9912?' -> '12345'
    """
    cleaned = raw_swipe.strip()

    # Remove common start/end sentinels for magstripe strings.
    cleaned = cleaned.replace("%", "").replace("?", "").strip()

    digit_groups = re.findall(r"\d{4,}", cleaned)
    if digit_groups:
        # Often the most useful ID is one of the longer digit runs.
...
def supabase_insert(table: str, payload: Dict[str, Any]) -> list:
    url = f"{SUPABASE_URL}/rest/v1/{table}"
...
def get_last_swipe_today(card_id: str) -> Optional[Dict[str, Any]]:
    """
    Finds the most recent swipe for this card since today's UTC midnight.
    For a stricter local-time version later, you can compute local midnight instead.
    """
...
def determine_event_type(card_id: str) -> Tuple[str, Optional[Dict[str, Any]]]:
    ...
    last_type = last_today["event_type"]
    if last_type == "IN":
